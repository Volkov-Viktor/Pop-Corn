...
//------------------------------------------------------------------------------------------------------------
ABall::ABall()
	: Ball_State(EBall_State::Disabled), Prev_Ball_State(EBall_State::Disabled), Release_Timer_Tick(0), Rest_Test_Distance(0.0),
	Center_X_Pos(0.0), Center_Y_Pos(0.0), Ball_Speed(0.0), Prev_Ball_Speed(0.0), Ball_Direction(0.0), Prev_Ball_Direction(0.0),
	Testing_Is_Active(false), Test_Iteration(0), Ball_Rect{}, Prev_Ball_Rect{}, Parachute_Rect{}, Prev_Parachute_Rect{},
	Recent_Hits_Pos(0), Recent_Hits_Count(0)
{
}
//------------------------------------------------------------------------------------------------------------
...
bool ABall::Detect_Hits_Cycling()
{
	...
	if (Recent_Hits_Pos >= buffer_len)
		Recent_Hits_Pos = 0;  // Начинаем отсчет позиций в циклическом буфере заново

	++Recent_Hits_Count;

	if (Recent_Hits_Count < buffer_len)
		return false;  // Не подсчитываем повторения, пока не наберем buffer_len столкновений
	...
	for (curr_pos = 3; curr_pos < buffer_len; curr_pos++)
	{
		...
		{
			Recent_Hits_Count = 0;  // Начинаем собирать статистику снова, чтобы не учитывать предыдущие серии
			return true;
		}
	}
